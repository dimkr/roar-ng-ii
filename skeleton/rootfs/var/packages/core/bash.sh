...
download() {
	...
	do
		...
		download_file \
	http://ftp.gnu.org/gnu/$PKG_NAME/$PKG_NAME-$PKG_MAJOR_VER-patches/$file_name
		[ 0 -ne $? ] && return 1
	done
	...
	make_tarball_and_delete $PKG_NAME-$PKG_MAJOR_VER.001-$PKG_PATCH_VER \
	                        $PKG_NAME-$PKG_MAJOR_VER.001-$PKG_PATCH_VER.tar.xz
	[ 0 -ne $? ] && return 1
